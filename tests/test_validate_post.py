...
class TestValidatePost(unittest.TestCase):
    ...
    def test_valid_post_has_no_errors(self):
        html = f"<p>{self._long_enough_text()}</p><a href='#' data-paddle-price-id='pri_test123'>Buy</a>"
        self.assertEqual(validate_post(html), [])

    def test_too_short_flagged(self):
        html = "<p>word word word</p><a href='#' data-paddle-price-id='pri_test123'>Buy</a>"
        errors = validate_post(html)
        self.assertTrue(any("minimum is" in e for e in errors))

    def test_denylisted_phrase_flagged(self):
        html = f"<p>{self._long_enough_text('you should administer medicine')}</p><a href='#' data-paddle-price-id='pri_test123'>Buy</a>"
        errors = validate_post(html)
    ...
    def test_missing_product_link_flagged(self):
        ...
        errors = validate_post(html)
        self.assertTrue(any("data-paddle-price-id" in e for e in errors))
